import { STAGE_LABEL, type GardenState, type Stage } from "./garden";
...
/** Static SVG of the plant for a given stage (used by SEO/OG pages). */
export function plantSvg(stage: Stage, size = 240): string {
  ...
  const stem = (h: number) =>
    parts.push(`<rect x="117" y="${196 - h}" width="6" height="${h}" rx="3" fill="${GREEN_D}"/>`);
  const leaf = (x: number, y: number, dir: number, s = 1) =>
    parts.push(
      `<path d="M${x} ${y} q ${18 * dir} -10 ${30 * dir} 2 q -${14 * dir} 12 -${30 * dir} -2 Z" fill="${GREEN}" transform="scale(${s} ${s})" transform-origin="${x} ${y}"/>`,
    );
  ...
  switch (stage) {
    case "seed":
      parts.push(`<ellipse cx="120" cy="190" rx="9" ry="6" fill="${SOIL}"/>`);
      parts.push(`<circle cx="120" cy="188" r="3" fill="#caa46a"/>`);
      break;
    case "sprout":
      stem(26);
      leaf(120, 176, 1, 0.8);
...
/** A minimal, SEO-friendly public profile page. */
export function profileHtml(state: GardenState, origin: string): string {
  ...
  const svg = plantSvg(state.stage, 260);
  return `<!doctype html>
<html lang="it">
<head>
<meta charset="utf-8"/>
...
<body>
  <main class="card">
    ...
    <p style="margin-top:18px;font-size:12px;color:#5b6b61">🌱 Commit Garden — ogni push innaffia la tua pianta.</p>
  </main>
</body>
</html>`;
}
